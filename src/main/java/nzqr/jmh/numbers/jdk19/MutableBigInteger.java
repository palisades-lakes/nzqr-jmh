...
import static nzqr.jmh.numbers.jdk19.BigDecimal.INFLATED;
...
import nzqr.jmh.numbers.jdk19.SignedMutableBigInteger;
...
class MutableBigInteger {
     ...
     * specified BigIntegerJDK.
     */
    MutableBigInteger(BigIntegerJDK b) {
        intLen = b.mag.length;
    ...
    /**
     * Convert this MutableBigInteger to a BigIntegerJDK object.
     */
    BigIntegerJDK toBigInteger(int sign) {
        if (intLen == 0 || sign == 0)
            return BigIntegerJDK.ZERO;
        return new BigIntegerJDK(getMagnitudeArray(), sign);
    }

    /**
     * Converts this number to a nonnegative {@code BigIntegerJDK}.
     */
    BigIntegerJDK toBigInteger() {
        normalize();
    ...
    BigDecimal toBigDecimal(int sign, int scale) {
        ...
        // make a BigIntegerJDK object for the resultant BigDecimal object.
        if (len > 2 || (d < 0 && len == 2))
            return new BigDecimal(new BigIntegerJDK(mag, sign), INFLATED, scale, 0);
        long v = (len == 2) ?
    ...
    public String toString() {
        BigIntegerJDK b = toBigInteger(1);
        return b.toString();
    }
    ...
    void rightShift(int n) {
        ...
        if (nBits == 0)
            return;
        int bitsInHighWord = BigIntegerJDK.bitLengthForInt(value[offset]);
    ...
    /**
     * Returns a {@code BigIntegerJDK} equal to the {@code n}
    ...
    private BigIntegerJDK getLower(int n) {
        if (isZero()) {
            ...
            while (len > 0 && value[offset+intLen-len] == 0)
                len--;
            int sign = len > 0 ? 1 : 0;
            return new BigIntegerJDK(Arrays.copyOfRange(value, offset+intLen-len, offset+intLen), sign);
        }
    }
    ...
    MutableBigInteger divide(MutableBigInteger b, MutableBigInteger quotient, boolean needRemainder) {
        if (b.intLen < BigIntegerJDK.BURNIKEL_ZIEGLER_THRESHOLD ||
                intLen - b.intLen < BigIntegerJDK.BURNIKEL_ZIEGLER_OFFSET) {
    ...
    MutableBigInteger divideKnuth(MutableBigInteger b, MutableBigInteger quotient, boolean needRemainder) {
        if (b.intLen == 0)
            throw new ArithmeticException("BigIntegerJDK divide by zero");
    ...
    MutableBigInteger divideAndRemainderBurnikelZiegler(MutableBigInteger b, MutableBigInteger quotient) {
        ...
        if (r < s) {
            ...
            int m = 1 << (32-Integer.numberOfLeadingZeros(s/BigIntegerJDK.BURNIKEL_ZIEGLER_THRESHOLD));
    ...
    private MutableBigInteger divide2n1n(MutableBigInteger b, MutableBigInteger quotient) {
        ...
        // step 1: base case
        if (n%2 != 0 || n < BigIntegerJDK.BURNIKEL_ZIEGLER_THRESHOLD) {
    ...
    MutableBigInteger sqrt() {
        ...
        if (bitLength() <= 63) {
            ...
            long v = new BigIntegerJDK(this.value, 1).longValueExact();
            ...
            double d = new BigIntegerJDK(xk.value, 1).doubleValue();
            BigIntegerJDK bi = BigIntegerJDK.valueOf((long)Math.ceil(Math.sqrt(d)));
    ...
    MutableBigInteger mutableModInverse(MutableBigInteger p) {
        ...
        if (isEven())
            throw new ArithmeticException("BigIntegerJDK not invertible.");
